<template>
    <div class="question">
      ...
      <navigation/>
      <div
        v-for="(question, index) in questionForm.questions"
        :key="question.questionId"
      >
        <!--单选-->
          <div style="width: 96%;margin: 0 auto;">
              <van-radio-group v-if="question.questionType==0 || question.questionType==2"
       ...
       <!-- 复选-->
        <div @click = 'checkboxChange($event)' style="width: 96%;margin: 0 auto;">
          <van-checkbox-group v-if="question.questionType==1"
            ...
            <div class="question_title"
              ...
              {{question.questionId+". "+question.questionContent}}（可复选）
            </div>
            <div class="choose">
              <van-checkbox
                :name="ans"
      ...
      data() {
          ...
          var _this = this;
            ...
            var postData = {
               'paper_type':'risk',
						...
						_this.$ajax.post('/adequacy/qryRiskResult',qryData).then(function (res) {
							if(res.data.error_no == 0){
							  if(res.data.paper_answer != undefined){
        ...
        onClickRight() {
          this.$router.push({name:'MyBussiness'})
              ...
              _this.loadingbox = true;
						this.$ajax.post('/adequacy/saveRiskAnswer',savaData).then(function (res) {
							_this.loadingbox = false;
							if(res.data.error_no == 0){
							var data = {
								"accept_no": _this.acceptNo,
								...
								_this.$ajax.post('/flow/updateAccept', data).then(function (res) {
								 ...
								 _this.quit = true;
								 _this.content = res.data.error_info;
							}else if(res.data.error_no == -2){   //踩雷
								_this.loadingbox = false;
								// _this.isOut = "true";
								_this.quit = true;
								_this.content = "您当前为风险承受能力最低类别的投资者，禁止开立账户，如有疑问请联系我司客服人员，联系电话："+sessionStorage.getItem('broker_hotline');
							}else{
								_this.show_content = true;
								_this.error_content = res.data.error_info;
							}
